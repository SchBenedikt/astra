...
export default function Logger({ filter = "none" }: LoggerProps) {
  ...
  // Neue Filterung für interne Tool Response Nachrichten
  const visibleLogs = logs.filter(log => {
    if (
      typeof log.message === "object" &&
      log.message !== null &&
      "excludeFromResponse" in log.message &&
      (log.message as any).excludeFromResponse === true
    ) {
      return false;
    }
    return true;
  ...
  return (
    <div className="logger">
      <ul className="logger-list">
        {visibleLogs.filter(filterFn).map((log, key) => {
          return (
